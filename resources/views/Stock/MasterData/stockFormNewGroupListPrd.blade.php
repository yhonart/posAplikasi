                    ...
                    <td class="p-0">
                        @foreach($groupProdList as $gP)
                            @if( $gP->cos_group == $mG1->idm_cos_group AND $gP->size_product == $mPrd->product_size)
                                <input type="text" class="form-control form-control-sm rounded-0 EDIT-PRICE" name="editPriceSell" id="editPriceSell" value="{{$gP->price_sell}}" onchange="saveToDatabase(this,'m_product_price_sell','price_sell','{{$gP->idm_price_sell}}','idm_price_sell','{{$mPrd->core_id_product}}')">
                            @endif
                        @endforeach
                    </td>
                @endforeach
                <td class="p-0">
                    <button type="button" class="btn btn-sm btn-danger DELETE-PRICE-SIZE float-right btn-flat" data-id="{{$mPrd->core_id_product}}" data-size="{{$mPrd->product_size}}"><i class="fa-solid fa-xmark"></i></button>
