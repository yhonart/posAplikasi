    ...
    <tbody>
        ...
        <tr data-id="{{$pL->idinv_stock}}">
            <td class="p-0" colspan="3" width="37%">
                <a href="#" class=" font-weight-bold text-black PRODUK-ITEM" data-idInv="{{$pL->idinv_stock}}">{{$pL->product_name}}</a>                
            </td>
    ...
    $(document).ready(function() {
        ...
        document.addEventListener('keydown', function(event) {
            if (event.key === 'ArrowDown') {
                ...
                functEnter(selectedId);
            }
        });

        $('.PRODUK-ITEM').on('click', function (e) {
            e.preventDefault();
            let selectedId = $(this).attr('data-idInv');
            functEnter(selectedId);
        });

        function functEnter(selectedId){
